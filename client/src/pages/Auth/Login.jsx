...
const Login = () => {
    ...
    Cookies.set("auth", JSON.stringify(authData), {
        expires: 7,
    });         if (authData.user.role === 1) { // 1 is Admin
        // If user came from a specific page, go there, otherwise go to admin dashboard
        navigate(location.state || "/admin/dashboard");
    }           else { // 0 is User
        // If user came from a specific page, go there, otherwise go to user dashboard
